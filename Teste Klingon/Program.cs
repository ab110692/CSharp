...
namespace Teste
{
    class Program
    {
        ...
        static readonly string letrasFoo = "slfwek";
        static readonly string letrasOrder = "kbwrqdnfxjmlvhtcgzps";
        ...
        /// <summary>
        /// Primeiramente, as letras Klingon são classificadas em dois grupos: as letras s, l, f, w e k 
        ///são chamadas "letras tipo foo", enquanto que as demais são conhecidas como "letras tipo bar".

        ///Os linguistas descobriram que as preposições em Klingon são as palavras de 3 letras que terminam
        ///numa letra tipo bar, mas onde não ocorre a letra d. 
        ///Portanto, é fácil ver que existem 63 preposições no Texto A.
        /// </summary>
        /// <param name="text"></param>
        static void TextoKlingon(string text)
        {
            int prep = 0;
            text.Split(' ').ToList().ForEach(a =>
            {
                if (a.Length == 3 &&
                !a.Contains('d') &&
                letrasFoo.IndexOf(a.Last()) == -1)
                {
                    prep++;
                }
            }
            );
            Console.WriteLine($"Quantidade de preposições: {prep}");
        }

        /// <summary>
        /// Verbos em Klingon
        /// Um outro fato interessante descoberto pelos linguistas é que, no Klingon, os verbos sempre 
        /// são palavras de 8 ou mais letras que terminam numa letra tipo foo.Além disso, se um verbo 
        /// começa com uma letra tipo bar, o verbo está em primeira pessoa.

        /// Assim, lendo o Texto A, é possível identificar 26 verbos no texto, dos quais 23 estão em primeira
        /// pessoa.
        /// </summary>
        static void TextVerbo(string text)
        {
            int verb = 0;
            int verbF = 0;
            text.Split(' ').ToList().ForEach(a =>
            {
                if (a.Length >= 8 &&
                letrasFoo.IndexOf(a.Last()) >= 0)
                {
                    verb++;
                    if (letrasFoo.IndexOf(a.First()) == -1)
                    {
                        verbF++;
                    }
                }
            ...
            Console.WriteLine($"Quantidade de verbos: {verb}");
            Console.WriteLine($"Quantidade de verbos em primeira pessoa: {verbF}");
        }

        static void TextOrder(string text)
        {
            List<string> t = text.Split(' ').ToList().OrderBy(a => a, new CustomSorter()).ToList();

            /*{
                return (letrasOrder.IndexOf(a[0]) * 1000) 
                + (letrasOrder.IndexOf(a[1]) * 100)
                + (letrasOrder.IndexOf(a[2]) * 10);
            }).ToList();*/

            /*t.ForEach(a=> Comparer
            {
                int x = (letrasOrder.IndexOf(a[0]) * 5)
                + (letrasOrder.IndexOf(a[1]) * 4)
                + (letrasOrder.IndexOf(a[2]) * 3);
        ...
        public class CustomSorter : IComparer<string>
        {
            ...
            {
                var chars = "kbwrqdnfxjmlvhtcgzps";
                if (chars.IndexOf(x[0]) < chars.IndexOf(y[0]))
                    return -1;
                return chars.IndexOf(x[0]) > chars.IndexOf(y[0]) ? 1 : 0;
            }
        }

        static void NumberKlingon(string text)
        {
            ...
            text.Split(' ').ToList().ForEach(a =>
            {
                var charList = a.ToCharArray().ToList();
                ...
                for (int i = 0; i < charList.Count; i++)
                {
                    ...
                    int baseCalc = 20;
                    int order = letrasOrder.IndexOf(charList[i]);
                    ...
                    {
                        total += (long)Math.Pow(baseCalc, i) * order;
                    }
                    //Console.WriteLine($"{charList[i]} - {order} - {Math.Pow(baseCalc, i)} - {total}");
                }
            ...
            Console.WriteLine($"números bonitos distintos: {num}");
        }
    }
}
